...
#system.time(
  ...
  for(j in 1:length(files)){
    ...
    tileGrid <- c(ceiling(dim[1]/20000), ceiling(dim[2]/20000)) #check how many tiles no larger than 20000px in either direction we can get (to do: check maximum feasible tile size on other laptops as well)
